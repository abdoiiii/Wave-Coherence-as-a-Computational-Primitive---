...
### 5.3 Five Corrective Findings

Test development produced failures that revealed design constraints. The first three emerged during the initial 10-test run; the fourth emerged when extending to 16 tests; the fifth emerged when adding the channel energy diagnostic (Test 23):
...
**Finding 5: Absolute Coherence Conflates Fundamental with Harmonics (Test 23)**

When computing channel energy distribution using |cos(n × Δθ)| (absolute value), entities at 120° spacing produce perfect |coherence| = 1.0 at n=3, 6, 9, and 12 equally — all integer multiples of the fundamental. The initial implementation of the η diagnostic could not distinguish the fundamental harmonic from its overtones, causing the test to report the wrong dominant channel.

The fix: use **signed** mean coherence (preserving the sign of cos) rather than absolute value. At the fundamental harmonic n=3, all pairs in a triadic group produce coherence +1.0 (aligned). At non-fundamental harmonics like n=1 and n=2, the signed coherence is -0.5 (anti-aligned). Only at multiples of the fundamental (n=6, 9, 12) does signed coherence also reach +1.0 — but the fundamental is identified as the **lowest** such n.

**Design rule:** When identifying the dominant structure in a dataset, use signed coherence to find the fundamental harmonic. Absolute coherence is appropriate for energy distribution display (the η profile) but not for fundamental detection. The fundamental harmonic of a group is the lowest n where mean signed coherence across all pairs exceeds the alignment threshold.
